...
module.exports.execute = async (client, message, args) => {
    ...
    if (!message.member.hasPermission("ADMINISTRATOR")) {
        ...
        message.channel.send(`🎵 ${message.author}, geçmesi için **${currentVideoVotes.num}/${skipCount}** oy verdiniz!`);
        ...
        if (currentVideoVotes.num >= skipCount) {
            message.reply(`⏩ **${queue.videos[0].title}** değiştirildi!!`);
        ...
        message.reply(`⏩ **${queue.videos[0].title}** değiştirildi. Değitiren: **${message.author.tag}**!`);
        queue.connection.dispatcher.end();
        return;
    }
}
